...
public class ArticleS
{
    ...
    /// <summary>
    /// Main feature, random ArticleS from desired root category. 
    /// Important: recursing and may be slow sometimes. (10-20secs)
    /// </summary>
    /// 
    public ArticleS RandomPageFromCategory(string categoryTitle, string rootCategoryTitle, string userID)
    {
        ...
        else
        {
            ...
            if (def.Seconds > 15)
            {
                ...
                sb.Append("ArticleS found after " + callCounter + " calls to wiki \r\n\r\n");
        ...
        string ArticleSTitle = ResponseURI.Replace("https://en.wikipedia.org/wiki/", "");
        callCounter++;

        string ResponseText;
        HttpWebRequest myRequest2 =
        (HttpWebRequest)WebRequest.Create("https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=&format=json&titles=" + ArticleSTitle);
        ...
        //check for issues with ArticleS   
        ...
        PhotoUrl = getPhotoForArticleS(title);
    ...
    public string getPhotoForArticleS(string title)
    {
    ...
    private string GetInfoNearByWithImgs(string lat, string lng, string radius)
    {
        ...
        var ArticleSID = dig["pageid"];
        var img = dig["thumbnail"]["source"];


        myRequest =
       (HttpWebRequest)WebRequest.Create("https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=1&format=json&pageids=" + ArticleSID);
        using (HttpWebResponse response = (HttpWebResponse)myRequest.GetResponse())
        {
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
        ...
        var ArticleS = root["query"]["pages"].First.First;

        var content = ArticleS["extract"];
        var id = ArticleS["pageid"];
    ...
    private string GetInfoNearBy(string lat, string lng, string radius)
    {
        ...
        var ArticleSID = JsonConvert.SerializeObject(dig);
    ...
    /// </summary>
    /// 
    private int GetViews(string ArticleSTitle)
    {
        ...
        string url = "https://wikimedia.org/api/rest_v1/metrics/pageviews/per-ArticleS/en.wikipedia/all-access/all-agents/" + ArticleSTitle + "/daily/" + timeStamp + "/" + timeStamp;
        string ResponseText;
    ...
    /// </summary>
    /// 
    private string FirstSentence(string ArticleSTitle)
    {
        //fix for first sentence wiki api function
        //ArticleSTitle = ArticleSTitle.Replace("%C3%A9", "é"); 
        //ArticleSTitle = ArticleSTitle.Replace("%E2%80%93", "-");

        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=true&exsentences=1&format=json&titles=" + ArticleSTitle);
        using (HttpWebResponse response = (HttpWebResponse)myRequest.GetResponse())
        {
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
        ...
        var content = ArticleS["extract"].ToString();
        //var id = ArticleS["pageid"];
        //var title = ArticleS["title"];
    ...
    /// 
    private string OnlyIntro(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=1&format=json&titles=" + ArticleSTitle); //&explaintext= for clean text
        using (HttpWebResponse response = (HttpWebResponse)myRequest.GetResponse())
        {
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
    ...
    /// 
    private string AllText(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://en.wikipedia.org/w/api.php?action=query&prop=extracts&format=json&titles=" + ArticleSTitle);
        using (HttpWebResponse response = (HttpWebResponse)myRequest.GetResponse())
        {
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
    ...
    /// </summary>
    ///
    private string hasBirthDate(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=claims&titles=" + ArticleSTitle);
        using (HttpWebResponse response = (HttpWebResponse)myRequest.GetResponse())
        {
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
    ...
    /// 
    private string hasDeathDate(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=claims&titles=" + ArticleSTitle);
    ...
    ///
    private string hasCauseOfDeath(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=claims&titles=" + ArticleSTitle);
    ...
    ///
    private bool isPerson(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=claims&titles=" + ArticleSTitle);
    ...
    /// </summary>
    ///
    private bool isEvent(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=claims&titles=" + ArticleSTitle);
    ...
    /// <summary>
    /// If the ArticleS is about animel, function returns emoji of the animal
    /// </summary>
    /// 
    private string isAnimal(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=claims&titles=" + ArticleSTitle);
    ...
    /// <summary>
    /// get ArticleS short description for notification
    /// </summary>
    /// 
    private string hasDescription(string ArticleSTitle)
    {
        string ResponseText;
        HttpWebRequest myRequest =
        (HttpWebRequest)WebRequest.Create("https://www.wikidata.org/w/api.php?format=json&action=wbgetentities&sites=enwiki&props=descriptions&titles=" + ArticleSTitle);
        using (HttpWebResponse response = (HttpWebResponse)myRequest.GetResponse())
        {
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
    ...
    private string MoreLike(string var1, string var2)
    {
        ...
        var ArticleS = root["query"]["search"].ElementAt(randomNum);

        var ArticleSID = ArticleS["pageid"];
       ...
       (HttpWebRequest)WebRequest.Create("https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro=1&format=json&pageids=" + ArticleSID);
        ...
        ArticleS = root["query"]["pages"].First.First;
